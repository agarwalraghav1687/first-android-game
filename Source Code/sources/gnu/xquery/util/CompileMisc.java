...
public class CompileMisc {
    ...
    public static Expression validateApplyValuesFilter(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        if (vproc.kind == 'R') {
            ...
            Expression init = new ApplyExp((Procedure) addOp, new Expression[]{referenceExp2, referenceExp3});
        ...
        ApplyExp doMap = new ApplyExp((Procedure) valuesMap, new Expression[]{lexp2, referenceExp5});
        doMap.setType(dotArg.getType());
        lexp2.returnContinuation = doMap;
        ReferenceExp referenceExp6 = new ReferenceExp(sequence);
        LetExp let2 = new LetExp(new Expression[]{new ApplyExp(sizeMethod, new Expression[]{referenceExp6})});
        let2.add(lastArg);
            ...
            if (lexp2.min_args == 3 && lexp2.max_args == 3) {
                ...
                if (type1 == null || NodeType.anyNodeTest.compare(type1) != -3) {
                    ...
                    if (lastArg.getCanRead()) {
                        ...
                        comp.letStart();
                        Declaration sequence = comp.letVariable(null, typeNodes, new ApplyExp((Procedure) CoerceNodes.coerceNodes, new Expression[]{exp1}));
                        comp.letEnter();
                        Method sizeMethod = typeNodes.getDeclaredMethod("size", 0);
                        ReferenceExp referenceExp = new ReferenceExp(sequence);
                        LetExp lastLet = new LetExp(new Expression[]{new ApplyExp(sizeMethod, new Expression[]{referenceExp})});
                        lastLet.addDeclaration(lastArg);
                        Expression function = exp.getFunction();
                        ReferenceExp referenceExp2 = new ReferenceExp(sequence);
                        lastLet.body = new ApplyExp(function, new Expression[]{referenceExp2, lexp2});

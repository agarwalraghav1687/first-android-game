...
public class CompileInvoke {
    ...
    public static Expression validateApplyInvoke(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
                    ...
                    if (kind == 'N') {
                        ...
                        if (slots.length * 2 == args.length - keywordStart || ClassMethods.selectApplicable(ClassMethods.getMethods(r5, "add", 'V', null, iproc.language), 2) > 0) {
                            ...
                            if (keywordStart < args.length) {
                                ...
                                ApplyExp applyExp = new ApplyExp((Procedure) methods[0], new Expression[]{arg0});
                                ae = applyExp;
                            }
                            ae.setType(r5);
                            ? r29 = ae;
                            if (args.length > 0) {
                                ...
                                if (sargs < args.length) {
                                    ...
                                    for (int i4 = sargs; i4 < args.length; i4++) {
                                        begin.add(visitor.visit((Expression) new ApplyExp((Procedure) Invoke.invoke, new Expression[]{new ReferenceExp(adecl), QuoteExp.getInstance("add"), args[i4]}), (Type) null));
                                    }
        ...
        ApplyExp applyExp4 = new ApplyExp((Procedure) new ArrayNew(elementType), new Expression[]{visitor.visit(sizeArg, (Type) Type.intType)});
        ...
        while (i7 < args.length) {
            ...
            begin2.add(new ApplyExp((Procedure) new ArraySet(elementType), new Expression[]{new ReferenceExp(adecl2), QuoteExp.getInstance(new Integer(index2)), visitor.visit(arg, elementType)}));
            index2++;
            i7++;
        }
